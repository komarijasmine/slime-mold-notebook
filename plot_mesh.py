...
edge_lines = [LineString([mesh_points[s], mesh_points[e]]) for s, e in mesh_edges]
edges_gdf = gpd.GeoDataFrame(geometry=edge_lines, crs="EPSG:3857")

point_objects = [Point(xy) for xy in mesh_points]
...
ax.set_xlim(meter_bounds[0], meter_bounds[2])
ax.set_ylim(meter_bounds[1], meter_bounds[3])
